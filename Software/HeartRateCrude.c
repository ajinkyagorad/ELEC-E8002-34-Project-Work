...
#define TIMER_INTERVAL_ACCEL_UPDATE     APP_TIMER_TICKS(2, APP_TIMER_PRESCALER) // 1000 ms intervals
...
void mpu_setup(void)
{
    ...
    p_mpu_config.smplrt_div = 1;   // Change sampelrate. Sample Rate = Gyroscope Output Rate / (1 + SMPLRT_DIV). 19 gives a sample rate of 50Hz
...
int main(void)
{
    ...
    accel_values_t accel_values;
		...
		int16_t y[3]={0,0,0},z[3] = {0,0,0},x[1]={0},w[2]={0,0},u=10;
		float filter[] = {1,1,1};
		
		double t = 0;
		printf("Index, Accelero, Gyro, Magneto");
		double t_spike=0,t_spike_last=0,bpm=0,bpm_old=0;
    // Enter main loop.
    for (;;)
    {
        power_manage();
        if(start_accel_update_flag == true)
        {
            mpu_read_accel(&accel_values);
						...
						x[0] = filter[0]*y[0]+filter[1]*y[1]+filter[2]*y[2];	
						// 400 as the threshold for x[0]
						if(x[0]>300){
							u = 100;
							if(t-t_spike_last>0.4){
								
								t_spike = t;
								bpm = 60.0/(t_spike-t_spike_last);
								t_spike_last = t_spike;
								}
						}else{
							u = 0;
						}
           ...
           /* printf("%f, %05d, %05d, %05d, %05d, %05d, %05d\r\n",
							t, accel_values.x, accel_values.y, accel_values.z,
